...
- 所有网络玩法模式继承 `ANetworkBattleGameMode`。
...
目标：确保 `ANetworkBattleGameMode` 是所有网络玩法的公共基类。

我负责：

- 维护 `ANetworkBattleGameMode` 的公共流程：连接、分配身份、Spawn、死亡、复活。
- 保持基类不写死具体玩法规则。
...
- 在编辑器中继续维护 `BP_NetworkBattleGameMode` / `BP_NetworkBattlePlayerController` 等蓝图资产。
